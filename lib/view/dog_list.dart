...
class _DogListPageState extends State<DogList> {
  List<Dogs> dogList = [];  //catsテーブルの全件を保有する
...
// catsテーブルに登録されている全データを取ってくる
  Future getDogsList() async {
    setState(() => isLoading = true);                   //テーブル読み込み前に「読み込み中」の状態にする
    dogList = await DbHelper.instance.selectAllDogs();  //catsテーブルを全件読み込む
  ...
  Widget build(BuildContext context) {
    return Scaffold(
      appBar: AppBar(title: const Text('犬一覧')),
      body: isLoading                               //「読み込み中」だったら「グルグル」が表示される
        ...
        child: ListView.builder(              // 取得したcatsテーブル全件をリスト表示する
          itemCount: dogList.length,          // 取得したデータの件数を取得
          itemBuilder: (BuildContext context, int index) {
            final dog = dogList[index];       // 1件分のデータをcatに取り出す
              ...
              child: InkWell(                 // cardをtapしたときにそのcardの詳細画面に遷移させる
                child: Padding(               // cardのpadding設定
                  ...
                  child: Row(                 // cardの中身をRowで設定
                      children: <Widget>[               // Rowの中身を設定
                        ...
                        Text(dog.dogname,style: const TextStyle(fontSize: 30),),     // catのnameを表示
                      ]
                  ),
                ),
                onTap: () async {                     // cardをtapしたときの処理を設定
                  await Navigator.of(context).push(   // ページ遷移をNavigatorで設定
                    MaterialPageRoute(
                      builder: (context) => DogDetail(id: dog.dogid!),   // cardのデータの詳細を表示するcat_detail.dartへ遷移
                    ),
                  );
                  getDogsList();    // データが更新されているかもしれないので、catsテーブル全件読み直し
                },
              ),
            );
          },
      ...
      floatingActionButton: FloatingActionButton(                   // ＋ボタンを下に表示する
        ...
        onPressed: () async {                                       // ＋ボタンを押したときの処理を設定
          await Navigator.of(context).push(                         // ページ遷移をNavigatorで設定
            MaterialPageRoute(
                builder: (context) => const DogDetailEdit()           // 詳細更新画面（元ネタがないから新規登録）を表示するcat_detail_edit.dartへ遷移
            ),
          );
          getDogsList();                                            // 新規登録されているので、catテーブル全件読み直し
        },
      ),
    );
  }
